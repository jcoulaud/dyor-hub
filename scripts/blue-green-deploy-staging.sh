...
# Configuration
APP_DIR="/var/www/dyor-hub-staging"
BLUE_PORT_WEB=4100
GREEN_PORT_WEB=4200
BLUE_PORT_API=4101
GREEN_PORT_API=4201
NGINX_CONFIG="/etc/nginx/sites-enabled/dyor-hub-staging"
...
cleanup_docker_resources() {
  {
    ...
    exec 1>>/var/log/dyor-hub-staging-cleanup.log 2>&1
...
log "Starting blue-green deployment for staging environment"
...
cd $APP_DIR
mkdir -p /tmp/dyor-hub-staging-backup/{apps/api,apps/web}
[ -f "./apps/api/.env" ] && cp "./apps/api/.env" "/tmp/dyor-hub-staging-backup/apps/api/.env"
[ -f "./apps/web/.env" ] && cp "./apps/web/.env" "/tmp/dyor-hub-staging-backup/apps/web/.env"

# Pull from staging branch instead of main
git fetch --depth 1 origin staging
git reset --hard origin/staging
mkdir -p apps/{api,web}
[ -f "/tmp/dyor-hub-staging-backup/apps/api/.env" ] && cp "/tmp/dyor-hub-staging-backup/apps/api/.env" "apps/api/.env"
[ -f "/tmp/dyor-hub-staging-backup/apps/web/.env" ] && cp "/tmp/dyor-hub-staging-backup/apps/web/.env" "apps/web/.env"
...
services:
  postgres:
    container_name: dyor-hub-staging-postgres
    env_file:
      - ./apps/api/.env
    restart: always
  api:
    container_name: dyor-hub-staging-$NEW_ENV-api
    ...
    depends_on:
      ...
      redis:
        condition: service_healthy
  web:
    container_name: dyor-hub-staging-$NEW_ENV-web
    ports: ['127.0.0.1:$NEW_PORT_WEB:3000']
    environment:
      - NEXT_PUBLIC_API_URL=https://api.staging.dyorhub.xyz
      - ENVIRONMENT=staging
    depends_on:
      api:
...
log "Building $NEW_ENV environment for staging"
...
if [ "$CHANGES_DETECTED" = "true" ] || ! docker image inspect dyor-hub-staging-api:latest >/dev/null 2>&1 || ! docker image inspect dyor-hub-staging-web:latest >/dev/null 2>&1; then
  log "Building Docker images for staging with cache optimizations"
  # Clean up old images to ensure we get a fresh build
  docker-compose -p dyor-hub-staging build --no-cache --build-arg BUILDKIT_INLINE_CACHE=1
...
log "Starting database services for staging"
docker-compose -p dyor-hub-staging up -d postgres redis
sleep 10
log "Starting $NEW_ENV services for staging"
docker-compose -p dyor-hub-staging up -d api web

# Wait for API service to become ready
log "Waiting for staging API service to become ready"
MAX_RETRIES=60
RETRY_COUNT=0
until curl -s "http://127.0.0.1:$NEW_PORT_API/health" | grep -q "ok" || [ $RETRY_COUNT -eq $MAX_RETRIES ]; do
  log "Waiting for staging API to be available... $(( RETRY_COUNT + 1 ))/$MAX_RETRIES"
  sleep 2
  RETRY_COUNT=$((RETRY_COUNT + 1))
done

if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
  log "WARNING: Staging API health check timed out, proceeding anyway"
else
  log "Staging API service is ready"
fi

# Wait for services to start
log "Waiting for all staging services to become ready"
sleep 10

# Switch traffic
log "Updating Nginx to point to staging $NEW_ENV environment"
...
log "Waiting for connections to drain from old staging environment"
sleep 30

# Stop old environment - first check if containers exist
log "Stopping old staging $CURRENT_ENV environment"
for container in "dyor-hub-staging-$CURRENT_ENV-web" "dyor-hub-staging-$CURRENT_ENV-api"; do
...
log "Staging deployment finished, cleanup will run in background" 