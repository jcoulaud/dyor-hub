  ...
  useEffect(() => {
    const fetchCalls = async () => {
      ...
      try {
        ...
        console.error('Failed to load token calls:', err);
    ...
    <Card className='bg-[#111] border-zinc-800/60 overflow-hidden'>
      <CardHeader className='py-3 px-4 flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <CardTitle className='text-base font-medium text-white'>Latest Token Calls</CardTitle>
        </div>
        <Link
          href='/token-calls'
      ...
      <CardContent className='p-0'>
        {isLoading ? (
          ...
          <p className='px-4 py-2 text-sm text-zinc-400'>No token calls found</p>
        ) : (
          <div>
            {calls.map((call) => {
              const multiplier =
                ...
                <Link
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <div className='min-w-0'>
                      <p className='text-sm text-zinc-200 truncate'>${call.token?.symbol}</p>
                      <div className='flex items-center gap-1'>
                        <p className='text-xs text-zinc-500'>called by</p>
                  ...
                  <div className='flex items-center'>
                    {multiplier && (
                      <p className='text-xs text-emerald-400 mr-2'>{multiplier.toFixed(1)}x</p>
                    )}
                    <p className='text-xs text-zinc-500 mr-2'>Target: {targetText}</p>
