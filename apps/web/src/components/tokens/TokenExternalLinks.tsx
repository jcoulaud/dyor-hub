    ...
    <div className={`flex flex-row flex-nowrap items-center gap-2 overflow-x-auto ${className}`}>
      {LINKS.map(({ name, icon, url }) => (
        <Link
          ...
          className='flex items-center justify-center gap-2 w-10 h-10 bg-black-800/50 backdrop-blur-sm border border-zinc-700/30 rounded-lg hover:bg-zinc-700/50 hover:border-blue-500/30 transition-all duration-200'
          title={name}>
